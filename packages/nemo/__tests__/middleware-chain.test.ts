...
describe('Middleware Chain Execution', () => {
  ...
  const createMockEvent = () => {
    return NemoEvent.from({} as any);
  };

  it('should continue execution after a "next" response', async () => {
    const firstMiddleware = vi.fn().mockImplementation(() => {
      return NextResponse.next();
    });
    
    const secondMiddleware = vi.fn().mockImplementation(() => {
      return undefined;
    });
  ...
  it('should carry modified headers between middleware functions', async () => {
    const firstMiddleware = vi.fn().mockImplementation((req) => {
      req.headers.set('x-test-header', 'test-value');
      return NextResponse.next();
    });
    
    const secondMiddleware = vi.fn().mockImplementation((req) => {
