...
<Tabs items={['Bun', 'PNPM', 'NPM', 'Yarn']}>
  <Tab value="Bun">```bash bun add next-easy-middlewares ```</Tab>
  <Tab value="PNPM">```bash pnpm add next-easy-middlewares ```</Tab>
  <Tab value="NPM">```bash npm install next-easy-middlewares ```</Tab>
  <Tab value="Yarn">```bash yarn add next-easy-middlewares ```</Tab>
</Tabs>
