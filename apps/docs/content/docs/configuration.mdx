...
<Tabs items={["Simple", "Route segment", "Multiple paths", "Custom"]}>
  <Tab value="Simple">
    ```ts title="middleware.ts"
    const middlewares = {
  ...
  <Tab value="Route segment">
    ```ts title="middleware.ts"
    const middlewares = {
      '/team{/:slug}?': // middleware functions for /team/<slug> route // [!code focus]
    };
    ```
  </Tab>
  <Tab value="Multiple paths">
    ```ts title="middleware.ts"
    const middlewares = {
      '/(home|settings)': // middleware functions for /home or /settings route // [!code focus]
    };
    ```
  </Tab>
  <Tab value="Custom">
    ```ts title="middleware.ts"
    const middlewares = {
      '/post{/:id(\\d+)}?': // middleware functions for /post/<id> route // [!code focus]
    };
    ```
  </Tab>
</Tabs>
